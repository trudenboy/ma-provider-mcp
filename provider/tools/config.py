...
def _register_provider_write_tools(
    ...
    *,
    ...
    async def trigger_provider_action(
        ...
        invoke_action_name = "invoke_provider_config_action"
        invoke_action = getattr(mass.config, invoke_action_name)
        entries = await invoke_action(instance_id, action_key)
        audit = _audit_id()
        LOGGER.info(
