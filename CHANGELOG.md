...
### Fixed
- Creating a player group now respects the current user's player filter.
- Saving a queue as a playlist requires playlist-edit permission.
- Track preview URLs are no longer exposed through MCP.
- Audio-analysis waveform queries honor the music-provider filter.
- Large mapping payloads are truncated during serialization.

## [2.1.8] - 2026-08-24

### Fixed
- Restricted users no longer see other players' queues in the full queue list.
- Unavailable music-provider instance IDs cannot be used to reach another
  allowed instance of the same provider.
- Log search matches only the redacted text returned to the client.
- Event capture now bounds payload size while the event is recorded.

## [2.1.7] - 2026-08-24

### Removed
- The optional player and queue MCP App is no longer offered. Clients continue
  to use `search_tools`, `get_tool_schema`, and `call_tool`.

## [2.1.6] - 2026-08-24

### Fixed
- Current Music Assistant player shuffle and repeat commands are available
  through the expected playback permissions.
...
### Security
- FastMCP is updated to 3.4.7, including the corrected OAuth proxy audience
  validation for private-key JWT authentication; Prefab UI remains on the latest
  stable 0.20.2 release.
...
- Reverse-synced upstream PR #6132 (WIP)
