...
from provider.tools import debug as debug_module
...
async def test_reload_timeout_populates_last_error(
    ...
    # Use ``from provider.tools import debug`` (not ``import provider.tools.debug as``):
    # the upstream import-path rewrite only translates ``from provider.`` imports.
    from provider.tools import debug as debug_mod  # noqa: PLC0415
...
async def test_reload_tool_uses_interactive_timeout() -> None:
    ...
    """
    from provider.tools._common import (  # noqa: PLC0415
        TIMEOUT_FAST,
        TIMEOUT_INTERACTIVE,
    )
