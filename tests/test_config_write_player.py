...
async def test_set_player_value_persists(mounted_config, mock_config_targets) -> None:
...
async def test_save_player_bulk_persists(mounted_config, mock_config_targets) -> None:
    async with Client(mounted_config) as client:
        result = await client.call_tool(
            "config_save_player",
...
async def test_save_dsp_persists(mounted_config, mock_config_targets) -> None:
    async with Client(mounted_config) as client:
        result = await client.call_tool(
            "config_save_dsp",
            {
...
async def test_save_dsp_dry_run_no_persist(mounted_config, mock_config_targets) -> None:
    async with Client(mounted_config) as client:
        result = await client.call_tool(
            "config_save_dsp",
            {
...
async def test_save_player_payload_over_64kb_rejected(mounted_config, mock_config_targets) -> None:  # noqa: ARG001
...
async def test_save_dsp_invalid_payload_rejected(mounted_config, mock_config_targets) -> None:  # noqa: ARG001
